    ...
    public static class FileSystem
    {
        ...
        */
        ...
        public static void CopyFile(string srcPath, string dstPath, bool overwrite)
        {
            ...
            using (var fsSrc = new FileStream(hSrc, FileAccess.Read, 0, hSrc.IsAsync))
            {
                ...
                using (var hDst = File.OpenHandle(dstPath, FileMode.Create, FileAccess.ReadWrite, FileShare.None, FileOptions.None, fileLength))
                {
                    if (fileLength != 0)
                    {
                        ...
                        var fsDst = new FileStream(hDst, FileAccess.Write, 0, hDst.IsAsync);
        ...
        public static bool CopyAttributes(SafeFileHandle hSrc, SafeFileHandle hDst)
        {
            ...
            unsafe
            {
                if (PInvoke.GetFileInformationByHandleEx(hSrc, MSFileSystem.FILE_INFO_BY_HANDLE_CLASS.FileBasicInfo, Unsafe.AsPointer(ref info_src), Convert.ToUInt32(Marshal.SizeOf(info_src)))
                && PInvoke.GetFileInformationByHandleEx(hDst, MSFileSystem.FILE_INFO_BY_HANDLE_CLASS.FileBasicInfo, Unsafe.AsPointer(ref info_dst), Convert.ToUInt32(Marshal.SizeOf(info_dst))))
                {
                    ...
                    return PInvoke.SetFileInformationByHandle(hDst, MSFileSystem.FILE_INFO_BY_HANDLE_CLASS.FileBasicInfo, Unsafe.AsPointer(ref info_dst), Convert.ToUInt32(Marshal.SizeOf(info_dst)));
                }
            }
